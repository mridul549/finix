...
async function sendPasswordResetMailHelper (req, res, key, url) {
    ...
    try {
        ...
        const mailOptions = {
            from: 'Finix <mridulverma478@gmail.com>',
            to: key,
            subject: `Finix Password Reset Mail`,
            html: renderedHTML
        }
...
module.exports.sendMail = async (key,role) => {
    ...
    try {
        ...
        const mailOptions = {
            from: 'Finix <mridulverma478@gmail.com>',
            to: key,
            subject: `${otpnew} is your Finix OTP Verification code`,
            html: renderedHTML
        }
...
const verifyAdmin = async (key,req,res) => {
    try {
        if(typeof key === 'string'){
            await Admin.updateOne({ email: key }, {
                $set: { verification: true }
            })
        } else {
            await Admin.updateOne({ mobile: key }, {
                $set: { verification: true }
            })
        }
    } catch (error) {
